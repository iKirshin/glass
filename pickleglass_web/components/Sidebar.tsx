...
import { ChevronDown, Github, HelpCircle, LucideIcon } from 'lucide-react';
...
const SidebarComponent = ({ isCollapsed, onToggle, onSearchClick }: SidebarProps) => {
    ...
    const navigation = useMemo<NavigationItem[]>(
        () => [
            ...
            },
            {
                ...
                icon: '/book.svg',
                isLucide: false,
                ariaLabel: 'Interview profile: résumé, competence, language level',
            },
            {
                name: 'Settings',
                href: '/settings',
    ...
    const bottomItems = useMemo(
        () => [
            ...
            {
                href: 'https://github.com/iKirshin/glass',
                icon: '/linkout.svg',
                text: 'GitHub',
                ariaLabel: 'InPro on GitHub (new window)',
                external: true,
            },
        ],
        []
    ...
    return (
        ...
        >
            <header className={`group relative h-6 flex shrink-0 items-center justify-between`}>
                {isCollapsed ? (
                    <Link href="/personalize" className="flex items-center">
                        <span className="mx-3 shrink-0 text-[15px] font-bold tracking-tight text-[#282828]">In</span>
                        <button
                    ...
                    <>
                        <Link href="/personalize" className="flex items-center">
                            <span className="mx-3 shrink-0 text-[15px] font-bold tracking-tight text-[#282828]">InPro</span>
                        </Link>
                        <button
                            onClick={toggleSidebar}
                            onKeyDown={e => handleKeyDown(e, toggleSidebar)}
            ...
            <nav className="flex flex-1 flex-col pt-8" role="navigation" aria-label="Main menu">
                ...
                {!isCollapsed && hasApiKey !== null && (
                    <div className="px-2.5 py-2 text-center">
                        <span className={`text-xs px-2 py-1 rounded-full ${hasApiKey ? 'bg-green-100 text-green-800' : 'bg-amber-100 text-amber-800'}`}>
                            {hasApiKey ? 'Local mode · ready' : 'Local mode · add an API key in the app'}
                        </span>
                    </div>
                )}
